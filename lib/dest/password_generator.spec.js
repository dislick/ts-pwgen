...
describe('PasswordGenerator', function () {
    describe('generate()', function () {
        ...
        it('should include at least one letter from each specified charset', function () {
            var pwgen = new password_generator_1.PasswordGenerator();
            pwgen.options = {
            ...
            for (var i = 0; i < 1000; i++) {
                var password = pwgen.generate().value;
                chai_1.expect(/[a-z]/.test(password), 'Did not include a lowercase letter').to.be.true;
                chai_1.expect(/[A-Z]/.test(password), 'Did not include an uppercase letter').to.be.true;
                chai_1.expect(/[0-9]/.test(password), 'Did not include a number').to.be.true;
            }
        });
    });
